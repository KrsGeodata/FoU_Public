...
const handlePropertySearch = async (propertyNumberToSearch: string = searchInput) => {
  const data = await fetchProperty(propertyNumberToSearch);
  if (!data?.length) {
  ...
  const incomingFeature = data[0]?.propertyboundary_geojson as GeoJSON.Feature | undefined;
  if (!incomingFeature || incomingFeature.type !== "Feature" || !incomingFeature.geometry) {
    setErrorMessage("Invalid GeoJSON Feature for the property");
    return;
  }

  // Merge/augment properties if you need custom props
  ...
  if (!loggedPropertyData.current) {
    ...
    console.log("Property boundary (feature):", propertyFeature);
    loggedPropertyData.current = true;
  }

  // Hand the Feature directly to Leaflet
  const newBoundary = L.geoJSON(propertyFeature as any, {
  ...
  if (mapRef.current) {
    ...
    if (bounds.isValid()) {
      mapRef.current.fitBounds(bounds, { maxZoom: MAX_ZOOM, padding: [20, 20] });
