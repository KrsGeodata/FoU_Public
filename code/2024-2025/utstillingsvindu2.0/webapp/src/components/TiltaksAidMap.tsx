...
import {
  ...
  fetchPropertyBoundaryAndAllowedArea,
...
const TiltaksAidMap = ({
  ...
  const handlePropertySearch = useCallback(
    async (propertyNumberToSearch: string = searchInput) => {
      ...
      try {
        const rawData = await fetchProperty(propertyNumberToSearch);
        if (!rawData?.length) {
        ...
        // Server returns FeatureCollections
        const boundaryCollection = first?.propertyboundary_geojson as FeatureCollection;
        const allowedCollection = first?.allowedbuildingarea_geojson as FeatureCollection | undefined;

        console.log("handlePropertySearch: " + boundaryCollection.type, allowedCollection?.type)

        // Extract polygon features for state


        if (!boundaryCollection) {
          setErrorMessage("Invalid or empty boundary geometry.");
          setInitialSearchSuccessful(false);
          return;
        }

        if (!loggedPropertyData.current) {
          console.log("Current property data:", first);
          console.log("Property boundaries:", first?.propertyboundary_geojson);
          console.log("Allowed area:", first?.allowedbuildingarea_geojson);
          loggedPropertyData.current = true;
        }

        // Keep app state as Feature[]
        setPropertyBoundaries(boundaryCollection?.features || []);
        setAllowedAreaGeoJson(allowedCollection?.features || []);
        ...
        setPropertyBoundary(boundaryLayer);

        const layersToFit: L.Layer[] = [boundaryLayer];

        // Draw allowed building area if present
        if (allowedCollection) {
          const allowedLayer = L.geoJSON(allowedCollection ?? allowedCollection, {
            style: { color: "green", weight: 2, dashArray: "4,4", fillOpacity: 0.08 },
          }).addTo(map);
          layersToFit.push(allowedLayer);
        }

        // Fit map to all layers
        const group = L.featureGroup(layersToFit);
        map.fitBounds(group.getBounds(), { maxZoom: MAX_ZOOM, padding: [20, 20] });
